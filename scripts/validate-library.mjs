...
assert(packageJson.bin?.["awesome-images"], "package must expose awesome-images CLI");
...
assert(skill.includes("npx github:flatkey-ai/awesome-images"), "skill must route users to CLI");
assert(skill.includes(flatkeyUrl), "skill must include Flatkey registration link");

const cliPath = path.join(root, packageJson.bin["awesome-images"]);
