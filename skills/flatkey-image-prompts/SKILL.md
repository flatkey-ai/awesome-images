...
`onboard` prompts for a Flatkey API key and stores it locally. If user has no key, send them to `https://console.flatkey.ai/keys`. CLI renders templates, calls Flatkey image API, and saves generated images locally. It reads saved key, `FLATKEY_IMAGE_API_KEY`, or `FLATKEY_API_KEY`.
